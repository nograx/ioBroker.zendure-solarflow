...
interface ZenDeviceDefinition {
  /**
   * productKey(s) as reported by the Zendure cloud API that identify this device, in their real (cloud-reported)
   * casing - see productKeys in admin/src/components/settings.tsx. Matching against a productKey of unknown casing
   * (e.g. from mDNS) must lower-case both sides, since productKeys[0] is also used verbatim as the productKey for
   * devices created directly from mDNS discovery.
   */
  productKeys: string[];
...
function findDeviceDefinitionByProductKey(productKey: string): ZenDeviceDefinition | undefined {
  ...
  return deviceDefinitions.find((definition) =>
    definition.productKeys.some((key) => key.toLowerCase() === normalizedProductKey),
  );
}
